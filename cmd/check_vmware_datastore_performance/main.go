...
func main() {
	...
	// by Nagios.
	nagiosExitState.CriticalThreshold = cfg.DatastorePerfPercentileSet().CriticalThresholdValues()
	nagiosExitState.WarningThreshold = cfg.DatastorePerfPercentileSet().WarningThresholdValues()
	...
	if dsPerfErr != nil {
		...
		defErrorHandler := func(exitState *nagios.ExitState, serviceState nagios.ServiceState, err error) {
		...
		case errors.Is(dsPerfErr, vsphere.ErrDatastoreIormConfigurationPropertyUnavailable):
			...
			defErrorHandler(&nagiosExitState, nagios.ServiceState{
