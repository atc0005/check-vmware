...
func main() {
	...
	cfg, cfgErr := config.New(config.PluginType{VirtualMachineList: true})
	switch {
	...
	filteredItems := FilteredItems{
		NumVMsRemainingAfterFiltering: len(filteredVMs),
		NumVMsExcludedByName:          numVMsExcludedByName,
		NumVMsExcludedByPowerState:    numVMsExcludedByPowerState,
		NumResourcePoolsEvaluated:     len(resourcePools),
		NumResourcePoolsIncluded:      len(cfg.IncludedResourcePools),
		NumResourcePoolsExcluded:      len(cfg.ExcludedResourcePools),
	}

	pd := getPerfData(vms, filteredItems)
	...
	switch {
	case len(filteredVMs) > 0:
		plugin.ServiceOutput = vsphere.VMListOneLineCheckSummary(
			nagios.StateOKLabel,
			filteredVMs,
			vms,
			resourcePools,
		...
		plugin.LongServiceOutput = vsphere.VMListReport(
			c.Client,
			vms,
			filteredVMs,
			cfg.IgnoredVMs,
	...
	default:
		log.Debug().Msg("No Virtual Machines remaining after filtering")
