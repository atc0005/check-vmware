...
func main() {
	...
	log := cfg.Log.With().
		...
		Str("included_folder_ids", cfg.IncludedFolders.String()).
		Str("excluded_folder_ids", cfg.ExcludedFolders.String()).
	...
	pd := vsphere.VMFilterResultsPerfData(vmsFilterResults)
	...
	switch {
	case vmsFilterResults.NumVMsAfterFiltering() > 0:
