...
func main() {
	...
	cfg, cfgErr := config.New(config.PluginType{SnapshotsOrphan: true})
	switch {
	...
	plugin.WarningThreshold = "any number of orphaned snapshots present"
	...
	for _, vm := range vmsWithSnapshots {
		...
		snapshotSets = append(
			...
			vsphere.NewSnapshotSummarySet(
				vm,
				vsphere.SnapshotThresholds{}, // FIXME: Unsure if this is the best approach?
			),
		)
	}
	...
	numVMsWithOrphanedSnapshots := snapshotSets.VMsWithOrphanedSnapshots()
	...
	pd := append(
		...
		[]nagios.PerformanceData{
			...
			// duplicate it here.
			{
				Label: "vms_with_orphaned_snapshots",
				Value: fmt.Sprintf("%d", numVMsWithOrphanedSnapshots),
			},
			{
				Label: "snapshots",
	...
	log = log.With().
		...
		Int("num_vms_with_orphaned_snapshots", numVMsWithOrphanedSnapshots).
		Logger()

	switch {

	case snapshotSets.IsOrphanCriticalState():

		log.Error().
			Msg("Snapshot sets contain a VM which has orphaned snapshots")

		plugin.AddError(vsphere.ErrSnapshotOrphanThresholdCrossed)

		plugin.ServiceOutput = vsphere.SnapshotsOrphanOneLineCheckSummary(
			nagios.StateCRITICALLabel,
			snapshotSets,
			vmsFilterResults,
		)

		plugin.LongServiceOutput = vsphere.SnapshotsOrphanReport(
			c.Client,
			snapshotSets,
			vmsFilterOptions,
			vmsFilterResults,
	...
	case snapshotSets.IsOrphanWarningState():
	...
	default:
		...
		log.Debug().Msg("No VMs found with orphaned snapshots")
